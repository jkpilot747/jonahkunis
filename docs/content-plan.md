...
- The domain went live partway through this session. `jonahkunis.com`'s
  DNS finished propagating (had been stuck on Wix's nameservers for
  several checks) — Vercel's dashboard confirmed "Valid Configuration"
  for both the apex domain and its `vercel.app` alias, and the user
  confirmed the site loads on their phone. Propagation from this
  environment's own resolver was still lagging behind at the time (a
  normal, resolver-specific delay, not a misconfiguration — confirmed by
  querying GoDaddy's authoritative nameservers directly and getting back
  the correct `216.198.79.1` record).
- **SEO, entirely untouched before this session — now wired up.** The
  site was still shipping literal `create-next-app` boilerplate
  (`title: "Create Next App"`, `description: "Generated by create next
  app"`) on every single page, no favicon beyond Next's default, no
  sitemap, no robots.txt, no Open Graph tags at all — meaning any link
  share (iMessage, Slack, LinkedIn, a Google search result) would have
  shown that placeholder text instead of anything about Jonah or his
  work. Fixed:
  - `app/layout.tsx`: real site-wide metadata (`metadataBase`, a
    `"%s | Jonah Kunis"` title template, description, Open Graph, Twitter
    card) that every route inherits by default.
  - `app/info/page.tsx`: its own `title`/`description`.
  - `app/work/[slug]/page.tsx`: a new `generateMetadata` function — real
    per-project title/description (markdown link syntax stripped to
    plain text), and an Open Graph image pointing at that project's
    actual cover photo. Verified live: Graduation's `<title>` reads
    "Graduation | Jonah Kunis", its OG image is the real
    `grad-b-32.jpg` cover, at the correct pixel dimensions.
  - `app/opengraph-image.tsx`: a generated default card (`next/og`'s
    `ImageResponse`) for `/` and `/info` — wordmark and tagline on white,
    tries to load real Instrument Sans Bold at request time and falls
    back silently to Satori's default sans if that fetch fails, so a
    network hiccup can never break the page.
  - `app/sitemap.ts` / `app/robots.ts`: generated `/sitemap.xml` and
    `/robots.txt`, the sitemap listing only visible projects (same
    "has images" rule the site itself uses to hide unfinished entries).
  - `npm run build` run clean afterward — new `/opengraph-image`,
    `/robots.txt`, `/sitemap.xml` routes all show up as static in the
    route list alongside the existing ones.
  - See docs/design-brief.md's new "SEO/metadata" note under Routes for
    the durable reference.
- **Two real mobile bugs, reported by the user testing on their own
  phone** (not caught by this session's earlier automated Chrome pass —
  see the prior session's note about that tooling's own resize
  unreliability; this pass used a differently-sized new tab as a
  workaround and got a real, if imprecise, mobile viewport):
  1. Opening a portrait (tall) photo in the lightbox left grid photos
     visible peeking in around it; landscape photos didn't show this.
     Root cause: the lightbox only set `document.body.style.overflow =
     "hidden"` to lock scroll, which doesn't reliably block scroll on
     mobile Safari — the underlying page can still rubber-band/drag
     under the fixed lightbox. A tall portrait image invites that drag
     gesture far more than a short landscape one sitting mostly
     surrounded by white space, which is why the leak read as
     orientation-specific rather than universal. Fixed with the standard
     technique: pin `<body>` to `position: fixed` at its current scroll
     offset while the lightbox is open, restore both the inline styles
     and scroll position on close (verified: closing after scrolling to
     y=500 restored exactly to y=500). Also added `overscroll-none` to
     the lightbox container itself as a second layer.
  2. The cursor-following nav arrow (added earlier this session, see
     above) showed as a phantom arrow stuck wherever a tap last landed —
     some mobile browsers synthesize a mousemove event on tap, which
     this feature had no reason to expect. Root cause understood, real
     fix applied: `handleNavMouseMove` now checks
     `matchMedia("(hover: hover) and (pointer: fine)")` and no-ops on
     anything without a real persistent-hover pointer, so the arrow
     never renders on touch at all. Tapping left/right still navigates
     exactly as before — confirmed via a manual tap in the browser tool
     that it still advances to the next image — since that's handled by
     the same `onClick` the arrow was only ever decorating, not gating.

---

## Status as of 2026-09-01

**The site is live.** `jonahkunis.com` resolves — DNS finished
propagating during this session (Vercel confirmed "Valid Configuration"
for both the apex domain and its `vercel.app` alias, and the user
confirmed the site loads on their own phone). Everything in this file's
session logs above is accomplished, committed on `main`, and pushed to
`origin`; the connected Vercel project deploys automatically on every
push.

**Every previously-open item is closed**, including the DNS/launch items
this section used to track (nameserver switch, A record, Vercel
verification) — see the session log above for the full trail. Also closed
this session: SEO was completely wired up from a from-scratch
`create-next-app` placeholder state (see "SEO, entirely untouched..."
above), and two real mobile bugs the user found on their own phone
(lightbox backdrop leak on portrait photos, a phantom nav-arrow on tap)
are fixed and verified.

**Open:**
1. **`www.jonahkunis.com` wasn't addressed.** Only the apex `@` A record
   ...
   not yet looked up.
2. **The mobile lightbox fixes were verified with an emulated viewport in
   this session's browser tooling, not a real phone.** The user reported
   the original bugs from their own phone but hasn't yet confirmed the
   fixes there — worth a real check next time they're on the live site.
3. **No search-engine submission yet.** `sitemap.xml`/`robots.txt` exist
   and are correct, but the sitemap hasn't been submitted to Google
   Search Console or Bing Webmaster Tools — that's a manual step outside
   this repo, not code, whenever the user wants to do it.

**Not yet started:**
- Ongoing content additions (new `raw/` photos for existing entries, a
